...
const OverviewView: React.FC<{ onCreateCampaign: () => void }> = ({ onCreateCampaign }) => {
    ...
    return (
        <div className="p-8 h-full overflow-y-auto animate-fade-in bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
            {/* Header Section */}
            <div className="flex justify-between items-end mb-10">
                <div>
                    <h1 className="text-4xl font-extrabold text-white tracking-tight uppercase">
                        Commande <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600">Système</span>
                    </h1>
                    <p className="text-slate-400 mt-2 font-light">Hub global d'orchestration et de supervision des intelligences artificielles.</p>
                </div>
                <button
                    onClick={onCreateCampaign}
                    className="group relative px-8 py-4 bg-white text-slate-900 font-bold text-lg rounded-none skew-x-[-10deg] hover:bg-purple-400 transition-all duration-300 shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:shadow-[0_0_30px_rgba(192,132,252,0.6)]"
                >
                    <span className="block skew-x-[10deg]">INITIER CAMPAGNE</span>
                    <div className="absolute inset-0 border border-white group-hover:border-purple-900 transform translate-x-1 translate-y-1 -z-10 transition-colors"></div>
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                {stats.map((stat, idx) => (
                    <div key={idx} className="bg-[#1e152a]/80 backdrop-blur-md border border-white/5 p-6 relative overflow-hidden group hover:border-purple-500/50 transition-colors">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-96">
                <div className="lg:col-span-2 bg-[#160e1b] border border-white/5 p-6 relative flex flex-col">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex space-x-2">
                            {['1H', '24H', '7J', '30J'].map(period => (
                                <button key={period} className="text-[10px] font-bold text-slate-500 hover:text-white px-2 py-1 border border-white/5 hover:border-white/20 transition-colors">{period}</button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex items-end justify-between px-4 space-x-2 opacity-50">
                        {[40, 65, 30, 80, 55, 90, 45, 70, 35, 60, 25, 50, 75, 40, 65].map((h, i) => (
                            <div key={i} className="w-full bg-gradient-to-t from-purple-900/50 to-purple-500/20 hover:to-purple-500/60 transition-all duration-300 rounded-t-sm relative group" style={{ height: `${h}%` }}>
                ...
                <div className="bg-[#160e1b] border border-white/5 p-6 flex flex-col">
                    ...
                    <div className="flex-1 space-y-4 overflow-hidden relative">
                        ...
                        {[
                            ...
                            <div key={i} className="flex items-start pl-6 relative">
                                <div className={`absolute left-[5px] top-1.5 w-1.5 h-1.5 rounded-full ${log.type === 'success' ? 'bg-green-500' : log.type === 'warning' ? 'bg-amber-500' : 'bg-blue-500'
                                    }`}></div>
                                <div>
                                    <span className="text-[10px] text-slate-500 block mb-0.5 font-mono">{log.time}</span>
                                    <span className="text-sm text-slate-300">{log.text}</span>
                                </div>
                            </div>
                        ))}
                        <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-[#160e1b] to-transparent pointer-events-none"></div>
                    </div>
                </div>
            </div>
        </div>
