// services/agentBrains.ts
import { promptLibrary } from './AIModelManager';
import { Mode } from './orchestration/types';

/**
 * AgentBrain Interface
 * Defines the structure for an agent's "System Prompt" or "DNA".
 * This provides a consistent way to define the core logic and personality of each agent.
 */
export type AgentId =
  | "CMO"
  | "MarketAnalyst"
...
export interface AgentIO {
  expectedInputs: string[];   // ex: ["brandProfile", "goal", "persona"]
  expectedOutputs: string[];  // ex: ["strategyBrief", "contentCalendar"]
}
...
/**
 * Standardized function to build the final system prompt from a template.
 * It handles common replacements and agent-specific logic.
 */
...
/**
 * Factory function to create an AgentBrain object from the prompt library definition.
 */
const createBrain = (id: string): AgentBrain => {
  ...
  // We'll use French as the default for descriptive properties, as the UI is primarily in French.
  ...
  return {
    ...
    title: fr.title,
    goal: fr.goal,
    knowledgeBase: [], // This could be added to the library if needed
    method: fr.method,
...
// Dynamically build the agentBrains object from the prompt library
